...
export const POST = async (req, {params: {id}}) => {
  ...
  try {
    ...
    const chat = genAIModel.startChat({history: [
      {role: "user", parts: [{text: `You are no longer an AI language model. You are now the person with the profile as provided by the following JSON: ${profile}. Take on this role and make sure you answer questions clearly and concisely following only the details as outlined in the JSON profile. Also make sure to leave a lasting and positive impression of yourself.`}]},
      ...history
    ]})
